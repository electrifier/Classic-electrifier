...
namespace Electrifier.Core {
	...
	public sealed class AppContext : ApplicationContext, IPersistent {
		...
		public void SaveConfiguration(string fileName) {
			...
			targetXmlDocument.AppendChild(targetXmlDocument.CreateComment("\n\n" +
			...
			XmlNode      rootNode    = targetXmlDocument.CreateElement(prefix, "Configuration", nmspURI);
			XmlAttribute versionAttr = targetXmlDocument.CreateAttribute(prefix, "FileVersion", nmspURI);
			versionAttr.Value        = "0.1";
			rootNode.Attributes.Append(versionAttr);
			// TODO: Add creation time to given rootNode

			// Append all xml node elements describing the actual configuration
			rootNode.AppendChild(CreatePersistenceInfo(targetXmlDocument, prefix, nmspURI));
			targetXmlDocument.AppendChild(rootNode);
		...
		public static XmlNode CreateXmlNodeFromForeignXmlDocument(XmlDocument targetXmlDocument,
			string prefix, string localName, string nmspURI, string foreignXmlDocumentStream) {
			...
			XmlNode xmlNode = targetXmlDocument.CreateElement(prefix, localName, nmspURI);
		...
		public XmlNode CreatePersistenceInfo(XmlDocument targetXmlDocument, string prefix, string nmspURI) {
			...
			XmlNode appContextNode = targetXmlDocument.CreateElement(prefix, "AppContext", nmspURI);
			XmlNode windowsNode    = targetXmlDocument.CreateElement(prefix, "OpenWindows", nmspURI);
			...
			foreach(IPersistent window in openWindows) {
				windowsNode.AppendChild(window.CreatePersistenceInfo(targetXmlDocument, prefix, nmspURI));
			}

			appContextNode.AppendChild(windowsNode);
